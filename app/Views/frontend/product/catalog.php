    ...
    <div class="breadcrumbs">
        ...
        <div class="breadcrumbs__url mt-4">
            <ul class="m-0 list-unstyled d-flex flex-wrap justify-content-center">
                ...
                <?php if (isset($child)) { ?>
                    <li class="breadcrumbs__url-item">
                        <a class="breadcrumbs__url-link" href="<?= base_url('danh-muc/' . $child['slug']) ?>"><?= $child['name'] ?></a>
                    </li>
                <?php } ?>
                <li class="breadcrumbs__url-item">
...
<div class="row">
    ...
    <div class="col-lg-9 col-12">
        ...
        <div class="catalog">
            ...
            <?php if (count($childCatalog)) { ?>
                <div class="catalog__child mb-4">
                    <h2 class="catalog__title">Danh mục con</h2>

                    <div class="row">
                        <?php foreach ($childCatalog as $item) { ?>
            ...
            <h2 class="catalog__title"><?= $row['name'] ?></h2>

            <?php if (count($productShowByCat)) { ?>
                <div class="catalog__filter">
                    <div class="row">
                        ...
                        <div class="col-8">
                            <select class="custom-select" name="filterCat" id="select__ajax">
                                <?= $select ?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="productFilterCat">
                    <div class="product product__th">
                        <div class="row">
                            <?php foreach ($productShowByCat as $item) { ?>
                ...
                <?php if (count($productShowByCat) >= 12) { ?>
                    <?= $pager->links() ?>
                <?php } ?>
            <?php } else { ?>
...
<script>
    $(function() {
        ...
        $(document).on('change', '#select__ajax', function(e) {
            var filterCat = $(this).val(); // lấy giá trị select 

            $.ajax({
                url: "<?= base_url(route_to('productCatalogFilter', $row['slug'])) ?>",
                type: "post",
                data: {
                    filterCat: filterCat,
                },
