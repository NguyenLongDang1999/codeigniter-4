...
<!-- Tiêu đề trang -->
<?= $this->section('title') ?>
Xem danh sách thùng rác sản phẩm
<?= $this->endSection() ?>
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title text-capitalize">Xem danh sách thùng rác sản phẩm</h4>
        </div>
    </div>
</div>
<!-- end page title -->
                            ...
                            <tr>
                                ...
                                <th>
                                    <?php if ($item['thumb'] == 'default-image.jpg') { ?>
                                        <img src="<?= base_url('uploads/' . $item['thumb']) ?>" class="img-fluid w-100" alt="<?= $item['name'] ?>">
                                    <?php } else { ?>
                                ...
                                <td>
                                    <ul class="m-0 list-unstyled">
                                        <li class="text-truncate" style="width: 150px">Danh mục: <?= isset($mcatalog->catalogGetName($item['catid'])['name']) ? $mcatalog->catalogGetName($item['catid'])['name'] : '' ?></li>
                                        <li class="text-truncate" style="width: 150px">Thương hiệu: <?= isset($mbrand->brandGetName($item['id'])['name']) ? $mbrand->brandGetName($item['id'])['name'] : '' ?></li>
                                    </ul>
                                </td>
                                <td><?= $item['created_at'] ?></td>
                                <td><?= $item['updated_at'] ?></td>
                                <td>
                                    <a href="<?= base_url(route_to('productRestore', $item['id'])) ?>" class="btn btn-xs btn-secondary"><i class="mdi mdi-backup-restore"></i></a>
                                    <a href="<?= base_url(route_to('productDelete', $item['id'])) ?>" onclick="return confirm('Bạn có chắn chắn muốn xóa vĩnh viễn sản phẩm này không ?')" class="btn btn-xs btn-danger"><i class="mdi mdi-trash-can-outline"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
