...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title text-capitalize">Xem danh sách danh mục</h4>
        </div>
    </div>
</div>
<!-- end page title -->
...
<div class="row">
    <div class="col-12">
        ...
        <a href="<?= base_url(route_to('catalogAdd')) ?>" class="btn btn-success waves-effect waves-light mb-2 text-capitalize">Thêm mới <i class="mdi mdi-plus-box"></i></a>
        <a href="<?= base_url(route_to('catalogRecycle')) ?>" class="btn btn-info waves-effect waves-light mb-2 text-capitalize">Thùng rác <i class="mdi mdi-trash-can"></i></a>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if($item['parentid'] == 0) {
                                        echo 'Là danh mục cha';
                                    } else {
                                        echo $mcatalog->catalogGetName($item['parentid'])['name'];
                                    }
                                        ?>
                                </td>
                                <td><a href="<?= base_url(route_to('catalogStatus', $item['id'])) ?>"><?= $status ?></a></td>
                                <td><?= $item['created_at'] ?></td>
                                <td><?= $item['updated_at'] ?></td>
                                <td>
                                    <a href="<?= base_url(route_to('catalogEdit', $item['id'])) ?>" class="btn btn-xs btn-secondary"><i class="mdi mdi-pencil"></i></a>
                                    <a href="<?= base_url(route_to('catalogTrash', $item['id'])) ?>" onclick="return confirm('Bạn có chắn chắn muốn đưa danh mục này vào thùng rác ?')" class="btn btn-xs btn-danger"><i class="mdi mdi-trash-can-outline"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
